...
{
    if(!empty($_POST['nomevent']) && !empty($_POST['typeevent']) && !empty($_POST['prix']) && !empty($_POST['dateheure']) && !empty($_POST['localisation']))
    {
		...
		$event = new Event(NULL, $nomevent,$typeevent, $prix , $dateheure, $localisation,$img,$description);
...
if(isset($_GET['delete']))
{
	...
	header("location:evennement-managment.php");
}


//display event
$events=$EventC->displayEvents();
			...
			<main class="content ">
				<div class="d-flex align-items-center justify-content-between mb-3" >
					 ...
					 <div id="popup-container-Admin" class="popup-container-Admin" style="width: 28%;">
						<a href="evennement-managment.php" id="close-popup-Admin"><i class="fas fa-times"></i></a>
						<div id="popup-content-Admin" class="">
							
							<h4 class="model-titel">Add a new Event </h4>
							<form action="" method="post" style="height: 400px;">
                                ...
                                <br>
								<label style="color: #5A8D9B; font-size:1em; cursor:pointer;"  for="img">image Event <i class="fas fa-image"></i></label>
								<input type="file" id="img" name="img">
								<br>
								<label for="typeevent">Id type event:</label>
                                <input type="number" name="typeevent" id="typeevent"  required>
                                <br>
                                ...
                                <input type="datetime-local" name="dateheure" id="dateheure" style="min-height:30px" required>
                                <br>
								...
								<br>
								<div class="d-flex align-items-center justify-content-between mb-3  ">
									<a href="evennement-managment.php" class="btn btn-secondary btn-block mt-3" style="border-radius: 5px;">Cancel</a>
							...
							<div class="card flex-fill">
								...
								<table class="table table-hover my-0">
									...
									<?php foreach ($events as $event): ?>
                                    	<tr>
                                    		<td   >
                                    			<span class="badge bg-danger" ><?php echo $event["idevent"]; ?></span>
                                    		</td>
											<td   >
											<img src="<?php echo $event["image"];?>" class=" rounded-circle mx-2 "  style="width: 45px; height: 45px;" ><span><?php echo $event["nomevent"]; ?></span>
                                    		</td>
											<td   >
                                    			<span><?php echo $event["nametypeevent"]; ?></span>
                                    		</td>
											<td   >
                                    			<span  ><?php echo $event["prix"]; ?></span>
                                    		</td>
											<td   >
                                    			<span class="badge bg-success" ><?php echo $event["dateheure"]; ?></span>
                                    		</td>
                                    		<td class=""><?php echo $event["localisation"]; ?></td>
                                    		<td>
                                    			 <a href="evennement-managment.php?update=<?= $event["idevent"]; ?>" style="margin:10px;" class="btn btn-primary open-popup-ElseWhere" name="modify" id="update-button">Modify</a>
                                    			 <a href="evennement-managment.php?delete=<?= $event["idevent"]; ?>" style="margin: 10px;" class="btn btn-danger" name="delete">Delete</a>
                                    		</td>
                                    	</tr>
									     <tr>
											<td>Description:</td>
...
if(isset($_GET['update']))
{
 ...
 echo '<script>
	...
	modelTitel.innerHTML="Update an Event";
