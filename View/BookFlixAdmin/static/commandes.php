...
$panierC = new panierC();
$commandes=$panierC->getAllCommandes();
if(isset($_GET['valider'])){
 $id=$_GET['valider'];
 $panierC->validerCommande($id);
 header('location:commandes.php');
}
if(isset($_GET['annuler'])){
    $id=$_GET['annuler'];
    $panierC->AnnulerCommande($id);
    header('location:commandes.php');
   }  
?>
			...
			<main class="content ">
				<div class="d-flex align-items-center justify-content-between mb-3">
					<div class="input-group" style="width: 300px !important;">
						<form action="" method="post" class="input-group" style="width: 300px !important;">
					...
					<div id="popup-container-Admin" class="popup-container-Admin" style="width: 28%;">
						<a href="livreur.php" id="close-popup-Admin"><i class="fas fa-times"></i></a>
						<div id="popup-content-Admin" class="">

							<h4 class="model-titel">Add a new livreur </h4>
				...
				<div class="container-fluid p-0">
					...
					<div class="row">
						<div class="col-12 d-flex">
									...
									<tbody>
                                        <tr>
										 <?php foreach ($commandes as $row) : ?>
											<tr>
												<td>
													<span class="badge bg-success"><?php echo $row["id"]; ?></span>
												</td>
												<td class="" ><?php echo $row["iduser"]; ?></td>
												<td class="" ><?php echo $row["total"]; ?></td>
                                                <td class="" ><?php echo $row["date_creation"]; ?></td>
                                                <td class="" >
                                                  <?php
                                                     if($row["valide"]==1)
                                                     {echo "validé";}
                                                ...
                                                <td>
													<a href="commandes.php?user-id=<?= $row["iduser"]; ?>" style="margin:10px;" class="btn btn-primary open-popup-ElseWhere" name="modify" id="update-button">Details</a>
                                                    <a href="commandes.php?valider=<?= $row["id"]; ?>" style="margin: 10px;<?php if($row["valide"]==1){echo "display:none";} ?>;" class="btn btn-success" name="delete">Valider</a>
                                                    <a href="commandes.php?annuler=<?= $row["id"]; ?>" style="margin: 10px;<?php if($row["valide"]==0){echo "display:none";} ?>;" class="btn btn-danger" name="delete">annuler</a>
												</td>
											</tr>
										 <?php endforeach; ?>
                                        </tr> 
                            ...
                            <div id="popup-container" style="  min-width: 350px; padding: 20px;" >
                                    <?php
                                     //read more
                                     ...
                                     ?>
									  <a href="commandes.php" id="close-popup"><i class="fas fa-times"></i></a>
									  <div id="popup-content"  style="  max-width: 900px;min-width:auto;height=500px">
                                           ...
                                           <table class="table table-hover my-0">
									            ...
									            <tbody>
									                  <?php foreach ($list as $row): ?>
										                   <tr>
											                   <td class=""name="product-name"><?php echo $row['name']; ?></td>
											                   <td  ><?php echo  $row['reference']; ?></td>
                                                               <td><span class="badge bg-danger" ><?php echo  $row['quantite']; ?></span></td>
