...
if (isset($_POST['submit'])) {
	if (!empty($_POST['name']) && !empty($_POST['phonenumber'])) {
        ...
        $livreur=new livreur(0, $name, $phonenumber);

		if (!empty($_POST['updating'])) {
			$id = $_POST['updating'];
            $livreur->setID($id);
			$livreurC->modifyLivreur($livreur);
		} 
        else 
        {
...
if (isset($_GET['delete'])) {
	$id = $_GET['delete'];
	$livreurC->deleteLivreur($id);
	header("location:livreur.php");
}

//display type event 
$liste=$livreurC->displayLivreur();
				...
				<div class="d-flex align-items-center justify-content-between mb-3">
					...
					<div id="popup-container-Admin" class="popup-container-Admin" style="width: 28%;">
						...
						<div id="popup-content-Admin" class="">
							...
							<form action="livreur.php" method="post" style="height: 250px;">							
								<label for="name">Name:</label>
								<input type="text" placeholder="" id="name" name="name">
								<br>
                                <label for="phonenumber">Phone Number:</label>
								<input type="number" placeholder="" id="phonenumber" name="phonenumber">
								<br>
								<div class="d-flex align-items-center justify-content-between mb-3  ">
									<a href="livreur.php" class="btn btn-secondary btn-block mt-3" style="border-radius: 5px;">Cancel</a>
								...
								<table class="table table-hover my-0">
									...
									<tbody>
										<?php foreach ($liste as $row) : ?>
											<tr>
												<td>
													<span class="badge bg-success"><?php echo $row["idlivreur"]; ?></span>
												</td>
												<td class="" name="product-name"><?php echo $row["name"]; ?></td>
												<td class="" name="product-name"><?php echo $row["phonenumber"]; ?></td>
                                                <td>
													<a href="livreur.php?update=<?= $row["idlivreur"]; ?>" style="margin:10px;" class="btn btn-primary open-popup-ElseWhere" name="modify" id="update-button">Modify</a>
													<a href="livreur.php?delete=<?= $row["idlivreur"]; ?>" style="margin: 10px;" class="btn btn-danger" name="delete">Delete</a>
												</td>
											</tr>
										<?php endforeach; ?>
									</tbody>
...
if (isset($_GET['update'])) {
	...
	modelTitel.innerHTML="Update livreur";
