<?php
include('../../../Core/typeeventC.php');
$typeeventC = new typeeventC();
session_start();

//add or update type event
if (isset($_POST['submit'])) {
	if (!empty($_POST['name'])) {
		$nametypeevent = $_POST['name'];

		if (!empty($_POST['updating'])) {
			$id = $_POST['updating'];
			$typeeventC->modifyTypeEvent($id, $nametypeevent);
		} else {
			$typeeventC->addTypeEvent($nametypeevent);
		}
		header("location:typeevent.php");
	}
}
...
if (isset($_GET['delete'])) {
	$id = $_GET['delete'];
	$typeeventC->deleteTypeEvent($id);
	header("location:typeevent.php");
}

//display type event 
$typeevents=$typeeventC->displayTypeEvent();
			...
			<main class="content ">
				<div class="d-flex align-items-center justify-content-between mb-3">
					...
					<div id="popup-container-Admin" class="popup-container-Admin" style="width: 28%;">
						<a href="typeevent.php" id="close-popup-Admin"><i class="fas fa-times"></i></a>
						<div id="popup-content-Admin" class="">

							<h4 class="model-titel">Add a new Event Type </h4>
							<form action="typeevent.php" method="post" style="height: 200px;">
								
								<label for="name">Name:</label>
								<input type="text" placeholder="" id="name" name="name">
								<br>
								<div class="d-flex align-items-center justify-content-between mb-3  ">
									<a href="typeevent.php" class="btn btn-secondary btn-block mt-3" style="border-radius: 5px;">Cancel</a>
								...
								<table class="table table-hover my-0">
									...
									<tbody>
										<?php foreach ($typeevents as $typeevent) : ?>
											<tr>
												<td>
													<span class="badge bg-success"><?php echo $typeevent["idtypeevent"]; ?></span>
												</td>
												<td class="" name="product-name"><?php echo $typeevent["nametypeevent"]; ?></td>
												<td>
													<a href="typeevent.php?update=<?= $typeevent["idtypeevent"]; ?>" style="margin:10px;" class="btn btn-primary open-popup-ElseWhere" name="modify" id="update-button">Modify</a>
													<a href="typeevent.php?delete=<?= $typeevent["idtypeevent"]; ?>" style="margin: 10px;" class="btn btn-danger" name="delete">Delete</a>
												</td>
											</tr>
										<?php endforeach; ?>
									</tbody>
...
if (isset($_GET['update'])) {
	...
	modelTitel.innerHTML="Update a new type event";
