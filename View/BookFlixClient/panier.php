...
if(!empty($idUser))
{
    $listpanier=$panierC->getPanier($idUser);
    if(isset($_POST['modify']))
     {
        ...
        $prix=$_POST['price'];
        $panierC->UpdatePanier($idpanier,$quantite,$prix);
        header('location:panier.php');
      }
      if(isset($_GET['panierdelete']))
      {
        $panierdelete=$_GET['panierdelete'];
        $panierC->deletePanier($panierdelete);
        header('location:panier.php');
      }
      $totalpanier=$panierC->getTotalPanier($idUser);
      if(isset($_GET['valider']))
      {
        $panierC->addCommande($idUser, $totalpanier);
        header('location:panier.php');
      }
      ...
      {
        echo'---------------------------------------------';
        $idproduit=$_SESSION['idproduct'];
        $prix=$productDb->get_prix($idproduit);
        $panierC->addpanier($idUser,$idproduit,$prix);
        $_SESSION['idproduct']=NULL;
        header('location:panier.php');
      }
...
<div class="container py-2">
    ...
    <div class="container">
        <div class="row">
          <?php
            if (!empty($panier)) {
                ...
                ?>
								<table class="table table-hover my-0" data-aos="fade-right" data-aos-delay="600">
									<thead>
										<tr>
											<th>Product Name</th>
											<th>Price</th>
									...
									<tbody>
									      <?php foreach ($listpanier as $row): ?>
										       <tr>
											       <td class=""name="product-name"><img src="<?php echo $productDb->selectPathImage($row['img1']); ?>" class=" rounded-circle mx-2 " name="product-img" style="width: 45px; height: 45px;" ><?php echo $row['name']; ?></td>
											       <td><span class="badge bg-success" ><?php echo $row['price'].' TND'; ?></span></td>
                                                   <td>
                                                    <div class = "purchase-info">
                                                       <form action="" method="post" class = "purchase-info">
                                                         <input style="display:none ;color:transparent;" type = "number" min = "0" value = "<?php echo $row['id'];?>" name="idproduit" id="idproduit">
                                                         <input style="display:none ;color:transparent;" type = "number" min = "0" value = "<?php echo $row['panier_id'];?>" name="idpanier" id="idpanier">
                                                         <input style="display:none ;color:transparent;"type = "number" min = "0" value = "<?php echo $row['price'];?>" name="price" id="price">
                                                         <input type = "number" min = "0" max="<?php echo $row['quantity']; ;?>" value = "<?php echo $row['quantite']; ?>" name="quantite" id="quantite">
                                                         <button type = "submit" class = "filter-btn product-details-btn  " name="modify" id="modify">   Modify <i class = "fas fa-shopping-cart"></i> </button>
                                                      </form>
                                                      <a  href="panier.php?panierdelete=<?php echo $row['panier_id'];?>" class = "filter-btn product-details-btn p-d-2"name="delete" >Delete</&>
                                                    </div>
											       </td>
											       <td class="d-none d-md-table-cell" ><?php echo $row['total'];; ?></td>
										       </tr>
										  <?php endforeach; ?>
        ...
        <div class="row">
            <table data-aos="fade-right" data-aos-delay="800">
                <thead>
                     <tr>
                        <th>Total </th>
                        <th>Operations</th>
                ...
                <tbody>
                    <tr>    
                      <td><?php if(isset($totalpanier)) echo $totalpanier;?></td>
                      <td>
                             <a href="panier.php?valider=1" type = "submit" class="btn btn-success " name="valider" id="valider">   Valider <i class = "fas fa-shopping-cart"></i> </a>
                      </td>
                    </tr>
                </tbody>
            </table>
